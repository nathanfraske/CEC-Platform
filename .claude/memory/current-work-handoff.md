...
merged main: firmware green-class (in_progress at check time); KiCad checks RED at the
host-suite leg = ONE deterministic test: test_corridor_model.TestPlacerCorridorEps.
test_phase2_default_placement_is_legal (residual 1) — INHERITED (fails on pipeline HEAD
standalone, whose last commit is a crashed-session wrap; pre-merge main PASSED it, so the
merge synthesis is clean). This red now masks main's older post-hygiene CI legs (D-11 class)
because hygiene fails first. NEXT PHASE (owner): pipeline pass — fix that test first, then
ruling-ladder improvements; the merged-in pipeline TODO carries an OWNER-GATED eps-rev3
SENSEC 40A current-path decision awaiting direction.

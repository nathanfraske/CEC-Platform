...
- **Waves**: W1 (running): [T] checkers (region-containment w/ teeth on current 12vhpwr flat sheet, sheet-bounds, symbol-inventory incl. DNP — netlist can hide a dropped DNP part, prose-preservation) + GND-ladder bus helper/mutator applied to hub+24pin ∥ [R] cec_pcb_reconcile.py (hub-standard committed sch/pcb pair = footprint-path calibration oracle). W2: compose root-lane-label extension + gen-module-beta.py (parametric cable-i2c family) proven on EPS. W3: PCIe×2 + 12vhpwr. W4: adversarial sweep + renders + docs + commit/push (branch claude/schematic-work-continue-59pw41 ONLY).

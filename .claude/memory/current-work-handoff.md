...
- **RESULT on the converged eps-8pin-r34 (96.2×37.2): 0% smaller — it's at its practical minimum.** No rigid slack (a 1mm shrink collides), and re-packing to shrink scrambles the placement enough that Freerouting drc_placement spikes to 46-63 (clips often IMPROVE 50→42-45, but clearance violations explode). The eps floor is ~96×35 (the committed hand board), reachable only by a from-scratch smart re-placement, NOT by shrinking r34. So the lever answers "is this placement size-optimal?" (yes) — the absolute floor needs the deferred synth placer as the re-pack engine (FOLLOWUPS).

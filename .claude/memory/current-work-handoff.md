...
- **LIVE PANEL auto-repoints (scripts/hub_run.sh)**: the launcher retargets a stable symlink `build/hub-LIVE` -> the run's OUT + ensures cec_dashboard.py is up on :8095 pointed at the symlink. The dashboard follows symlink retargets LIVE (abspath preserves the symlink, per-poll glob) -> every launch auto-repoints with no manual step (proven). Use `bash scripts/hub_run.sh [OUT] [HOURS] [CANDS]` to launch+repoint in one command. Panel: http://localhost:8095. Render via compose `routing` (up). NOTE: `timeout` on a docker CLI orphans the container -> the launcher uses `docker run -d --rm` (daemon-managed) + the script's own --hours budget, no outer timeout.

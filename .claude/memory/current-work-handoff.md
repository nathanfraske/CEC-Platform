...
- **FULL PIPELINE WIRED + RUN 2026-06-14** (scripts/hub_pipeline_run.py): place→materialize→route→check runs end-to-end on the Hub. `materialize_onto_reference` fixed the materialize gap (build_board output is NOT DSN-exportable → copy committed Hub + reposition, in a spawn subprocess to dodge the pcbnew Remove footgun). **BLOCKER FOUND: Freerouting does not route the Hub** — emits 1 wire (FR log: autoroute pass#1 "1 change"→pass#2 "0 changes"→stops), 216 unconnected; the identical cec_fr path routes cable modules to 556 tracks. Hub has 1 GND zone (not multi-zone). So an hour run is NOT worthwhile (reproduces 1-trace) — reported, not burned.
- **NEXT (owner goal) — levers to a CLEAN ROUTED Hub** (FOLLOWUPS, priority): (1) MV5 antenna-edge ESP seed; (2) power-loop cohesion sweep; (3) legalizer residual 4→0; (4) **the real blocker: get FR to route the Hub** — read build/frlog/logs/freerouting.log for why pass#1 makes 1 change (In2.Cu availability? keepout? netclass clearance too tight?), try residual-0 placement first, diff Hub-vs-eps DSN, or fall back to the cec_route pcbnew toolkit for the deterministic nets. The committed Hub (length 2359/gates pass) is the free calibration anchor once FR routes.

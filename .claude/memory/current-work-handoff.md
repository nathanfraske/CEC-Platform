...
- **NEXT (owner greenlight):** (1) reclaim the U1 dead space via Update-from-Library + a guided re-place (~36% target) — interactive, not blind; (2) wire the 2.5D thermal solver into the loop validity gate so workers can narrow rail pours / drop stitching vias keeping dT≤30 (owner-authorized; copper-mass lever, separate from outline size). Dashboard left up on :8095 for morning review.

...
- **HONEST STATE:** best valid board = clips=24/drc=11 (r9, kelvin-true, fully routed). The pin-level lever is built+validated but trades clips↔drc on this dense board, so it's not a standalone closer. Reaching the hand board's ~3/~3 (BOTH low) needs GLOBAL placement co-optimization (joint position+rotation+routing), a bigger build than the local region+orient the loop now has. `--loop-orient` stays opt-in (net-neutral).

...
_Updated 2026-06-30 (thread (E) PLACER pipeline on branch claude/placement-corridor, worktree /home/nathan/cec-placement — route-oracle grader + PCIe-3port grow + the intent-compiler, all below. Prior threads (C)/(D)/(A)/(B) remain as recorded.)._

## (E) Placer pipeline — route-oracle grader + 3-port grow + intent-compiler (2026-06-30, branch claude/placement-corridor)
Owner signed off two items: #1 the placer-autonomy milestone, #2 the PCIe-3port grow. State:
- **Route-oracle grader (SLICE-1a) — DONE + verified** (commit f73cace). `route_oracle_grade`/`adjudicate_candidates` in cec_synth_pipeline: grade a placement by ACTUALLY routing it (gate-clean recipe) + the full accept conjunction (kelvin AND diffpair AND drc-finishing AND foreign==0 AND thermal≤gate, fail-closed). Replaces the HPWL proxy. Verified: eps-n2 grades tier-0 clean, the lower-HPWL-but-broken eps-widegap-m grades tier-1 fail — oracle inverts the proxy, correctly. Opt-in (CEC_ROUTE_ORACLE=1), default-off, tests/test_route_oracle.py.
- **PCIe-3port GROW (#2) — primary win banked, secondary wall hit** (commit 6703b73). gen-pcie-condensed.py height is now per-SKU; 3-port H 44→56, BAND_Y=H/2 re-centers the sense row (2-port PROVEN byte-identical). Route-oracle: **foreign_on_pour 46t+11v → 0t/0v (the ratified LAYER BIND CLEARED)**, thermal fine. BUT the route doesn't complete — `SENSEC*_LO` kelvin fail.
- **Intent-compiler (SLICE-1b, #1) — CORE BUILT + TESTED** (commit 5271ac1). scripts/cec_placement_session.py `PlacementSession`: declarative region/assign partition over EXISTING synth_one primitives + a NEW gated `bounds` HARD-containment lever in legalize_pack/synth_one (partition=None PROVEN byte-identical → golden-safe). Verbs: region/half/assign + peripheral_ics/cable_parts/antenna_ics (net-BFS) + snapshot/rollback + compile→Candidate + grade→oracle + autonomy_search() harness. tests/test_placement_session.py 5/5 fast (inertness, hard-containment, TEETH=free-IC-forced-to-a-box-lands-in-it). Autonomy fan-out (agent proposes intents) is the NEXT increment.
- **CONVERGENT BLOCKER (the real next lever):** both #2's `SENSEC*_LO` AND #1's milestone-1 (a fresh-placed eps gate-clean) are blocked by the SAME thing — the INA238 LO Kelvin tap (IN-, **pad 9**) is REFUSED by synthesize_kelvin_taps' clearance guard: pad 9 is top-clustered next to IN+(pad 10) while the LO shunt terminal is at the bottom, so the straight LO stub clips foreign copper. Ruled out: partition / grow / pass-budget / pitch. Fix = re-seat the INA238 (or 1-bend LO tap / extend tap_channel_keepout). See memory `ina238-lo-tap-refusal-blocker.md` + docs/owner-queue.md. NOT yet committed/attempted — the clean checkpoint point.

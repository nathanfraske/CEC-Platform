...
- **NEXT (owner goal) — four levers to a CLEAN ROUTED Hub** (FOLLOWUPS, priority order, each validatable vs similarity/hub_terms/DRC): (1) MV5 antenna-edge ESP seed (biggest gap); (2) power-loop cohesion sweep; (3) legalizer residual 4→0 (clears the real-structural DRC); (4) route leg L1 (find_board hubs/ + route the materialized Hub → closes the 211 unconnected → full place→route→check vs the fab board, length 2359/gates the free calibration anchor).

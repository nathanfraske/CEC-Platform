...
class Bench:
    def __init__(self, port, baud, run_dir, analyze, module, analyzer, show_teleplot, tp_addr):
        ...
        self._man.write(f"# CEC bench run `{os.path.basename(run_dir)}`\n\n"
                        f"port {port} @ {baud}, module {module}, started "
    ...
    def _save_capture(self, block):
        ...
        with open(path, "w") as f:
            f.write("\n".join(block) + "\n")
        self._manifest("capture", f"`{name}` ({len(block)} lines)")
        print(f"\n[bench] saved captures/{name}")
        if self.analyze and self.analyzer:
            self._run_analyzer(path)

    def _run_analyzer(self, path):
        out = os.path.join(self.run_dir, "analysis")
        try:
            ...
            if r.returncode != 0 and r.stderr:
                print("[analyze] " + r.stderr.strip())
            self._manifest("analyze", os.path.basename(path))
        except Exception as e:
            print(f"[bench] analyze failed: {e}")

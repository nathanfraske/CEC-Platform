...
/* Emit a full state snapshot (both currents, optional raw values,
 * temp, load state, status flags). App-side: these are eps field names
 * over the shared teleplot engine. */
static void teleplot_emit_state(const cec_shared_state_t *state, bool include_raw)
{

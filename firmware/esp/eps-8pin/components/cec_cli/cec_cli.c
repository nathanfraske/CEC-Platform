...
static int handle_help(int argc, char **argv)
{
    ...
    printf("  %-*s  %s\n", (int)name_w, "help",
           "show this list, or 'help <command>' for one-line detail");
    for (size_t i = 0; i < s_command_count; i++) {
...
esp_err_t cec_cli_init(const cec_cli_command_t *commands, size_t count)
{
    ...
    /* Default USB buffers are fine now: TelePlot output diverts to the
     * UART transport (see cec_telemetry_init_uart in eps_main), so this
     * stream only carries CLI input + ESP_LOG + the occasional command
     * response. None of those need a bigger TX buffer. */

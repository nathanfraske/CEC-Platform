...
7. USB power-ingress mux and the KVM third stage (Sections 6.14 and 2.9, v1.6.0): spec-locked for the beta line but not yet on any as-built schematic. Every beta Standard module still carries the SS34 ORing diode (no TPS2121 ingress, no VBUS polyfuse), and the beta Hub's J_KVM pin 1 is still a raw +5VSB tap (no third cascade stage, no KVM polyfuse). The per-board part and refdes plan is `docs/usb-ingress-bom-delta-2026-07-24.md`; the schematic pass follows. Until it lands, the v1.6.0 interim bench rules of Section 2.9 apply on any bench with a suspect PSU (added 1.6.0).

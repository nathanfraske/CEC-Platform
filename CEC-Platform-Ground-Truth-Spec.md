...
Source selection is hardware, a priority ideal-diode OR (main 5V, then 5VSB, then a Hub-side external 5V-in — the internal USB-C VBUS for bench/dev, and the forensic wall-wart on a dedicated power jack per OQ-54), rather than a firmware switch. NOTE (v3.7): the wall-wart does NOT enter through the NanoKVM. The NanoKVM exposes only UART1 + GND + 3V3 (OQ-51), so the Hub-to-NanoKVM link is UART-only and the two boards are independently powered; the forensic source therefore plugs into the Hub itself. As built, the OR is a cascade of two TPS2121s (Section 2.7): U7 ORs main 5V > the U5 PSU-mux output, and U5 keeps the 5VSB/USB-C OR — i.e. priority main 5V > 5VSB > USB-C, with the forensic jack joining the USB-C-class input. The OQ-55 part search (TPS2116 / LTC4412-class) named the same TI PowerPath family; the cascade reuses U5's TPS2121 part. Firmware runs on the Hub MCU, which is powered by the rail it would be switching, so a firmware-controlled switch can deadlock the MCU if it selects a source that is not solid. The firmware instead reads which source is live on a rail-sense and sets the load budget and mode: full budget and normal monitoring on main 5V, the standby cap (OQ-2) on 5VSB, and a minimal LEDs-off forensic-extraction mode on the wall-wart. It may gate the main-5V enable once the PC is confirmed stably up, but the 5VSB fallback stays hardware-automatic. This is priority switching, with the OR isolating the idle inputs, and the rails are never paralleled to sum their capacity, which they do not need since main 5V alone is ample.

Back-feed isolation is the safety-critical rule. Every source feeds the Hub rail through its own ideal-diode mux input, and the rail must never push power back toward an idle source or into a dead system. In particular the forensic wall-wart, when it is the source, must not travel up the Hub's main-5V or 5VSB inputs into a dead PSU's rails and the motherboard. The cascaded TPS2121s (Section 2.7) provide that source-side reverse blocking on every input — the idle inputs are reverse-biased off — and the downstream D1 reverse-isolation Schottky walls off the +5V_HOLD reservoir; confirm the muxes' reverse blocking against the wall-wart-driven case. (The NanoKVM is no longer on the Hub's rail, so there is no inter-board 5V back-feed path to isolate — see below.)

The Hub and the NanoKVM are independently powered (revised v3.7). The NanoKVM exposes only UART1 + GND + 3V3 (OQ-51), so the inter-board aux header is UART-only and carries no 5V — there is no shared rail spanning the two boards, and the earlier "shared 5V conductor in either direction" framing is dropped. What the Hub's OR feeds is the Hub's own subsystem rail (+5VSB, post-mux). Whether the module fleet also rides that Hub rail (the whole-platform dual-feed that frees on-board LED brightness on a maxed build by moving the operating draw to main 5V in S0) or stays on 5VSB-only with the discretionary loads offloaded individually (per-load) is OQ-53.

Forensic recovery (the dead-system case, ties OQ-48), revised v3.7 for the UART-only link. Two sub-cases. (1) Host down but the PSU still has AC (a hang or a stuck POST): the Hub already runs on 5VSB and the NanoKVM on its own slot/USB power, so the Hub serves its persisted data over the UART and the NanoKVM egresses it over its own Ethernet/WiFi — no external power needed, and this is the common case. (2) Fully dead PSU: because the NanoKVM link is UART-only and carries no 5V (OQ-51), the wall-wart cannot power the Hub through the NanoKVM. The Hub is instead revived on its own external 5V-in — a CEC power-in jack on a rear bracket feeding the OR (OQ-54, now the REQUIRED forensic path rather than the optional general one) — while the NanoKVM is revived on its own externally accessible USB-C. Both then run as before: the Hub serves over the UART, the NanoKVM egresses, LEDs off, ~an amp total now split across the two independent supplies rather than one shared rail. The cost is the Hub rear-bracket jack; the gain is that the inter-board shared-5V conductor and its back-feed isolation are gone.
...
#     forensic : 5V into the Hub's OWN rear power-in jack (OQ-54)
#                — NOT via the NanoKVM (UART-only link, carries
#                no 5V); the OR's idle inputs block the dead PSU
#   The NanoKVM is independently powered; the inter-board aux
#   header carries UART(TX/RX) + GND + 3V3-ref only (no shared
#   rail). The Hub serves flash data over the UART; the NanoKVM
#   egresses over its own network.
#
...
| NanoKVM aux link | Reserved keyed header to the NanoKVM, DATA-ONLY: full-duplex 3.3V UART (TX/RX), ground, and the NanoKVM's 3.3V reference/presence line — the NanoKVM exposes only UART1 + GND + 3V3, so the two boards are independently powered and there is NO shared 5V feed (interrupts ride the UART in-band; no trigger GPIO). 4-pin. (local visual-and-electrical fusion, out-of-band egress; trust per OQ-50, form per OQ-51, Appendix C.7) |
...
**OQ-51: NanoKVM-to-Hub link form (RESOLVED form v3.7; baud + message set still open).** The NanoKVM hardware exposes only UART1 + GND + 3V3, which settles the open part: the physical link is a reserved keyed **4-pin aux header — full-duplex 3.3V UART (TX/RX), ground, and the NanoKVM's 3.3V reference/presence line — DATA-ONLY, with the two boards independently powered and NO shared 5V** (this reverts the Section 2.9 "power-plus-UART" idea). No fast-trigger GPIO is populated: interrupts ride the UART in-band (the NanoKVM's framebuffer-capture latency made a hardware trigger marginal anyway). Still to lock: the baud (921600 working, matching the Teleplot tooling and giving headroom for window egress) and the framed message set (fault events in; keyframe request and frozen flight-recorder window out; heartbeat both ways; correlation IDs for co-capture alignment). The Hub 3V3-ref pin may tie to a GPIO as a NanoKVM-present sense. See Appendix C.7 and Section 2.9.
...
**OQ-54: External forensic power-in (now the required forensic path, v3.7).** With the NanoKVM link reduced to UART-only (OQ-51), the NanoKVM can no longer feed 5V to the Hub, so the "wall-wart into the NanoKVM USB-C powers both boards over a shared rail" path is gone. For the fully-dead-PSU case the Hub must therefore carry its OWN external 5V-in: specify a **CEC power-in jack on a rear bracket** (5V, ~1A) that feeds the Hub's priority OR (joining the USB-C-class input) and revives the Hub for flash extraction; the NanoKVM is revived independently on its own externally accessible USB-C. Lock the jack type (barrel vs USB-C), its OR entry point, and whether it doubles as the bench/dev supply. The host-down-but-PSU-alive egress case needs no external power (Hub on 5VSB). See Section 2.9.

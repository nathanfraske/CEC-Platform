...
- [2026-06-14] **Hub placer pipeline is PROVEN at the placement level (test run 2026-06-14) — four levers remain to a CLEAN ROUTED Hub.** Baseline measured by the in-container test run (recipe in docs/placer-upgrade-2026-06-14/STATUS.md "HUB PIPELINE TEST RUN"): place→materialize→render→DRC→score all run; best residual 4, corridor_cross 0, **similarity 0.705**, **HPWL 1.25× the hand board** (was 1.84× pre-MV), hub_penalty 0.373. Remaining work, priority order — each independently validatable against the MV3 similarity / hub_terms / DRC numbers (so "did it help?" is measurable):
  ...
  4. **Route leg (plan L1) — WIRED (scripts/hub_pipeline_run.py) but FR DOES NOT ROUTE THE HUB (the real blocker).** The full place→route→check chain runs end-to-end; `materialize_onto_reference()` solved the materialize gap (build_board output is not DSN-exportable; copy the committed Hub + reposition instead). BUT Freerouting emits **1 wire** on the reposed Hub (FR log: `autoroute_pass #1: 1 change` then `#2: 0 changes` → declares the board already-routed and stops), leaving **216 unconnected** — while the identical cec_fr path routes the cable modules to 556 tracks. The Hub has ONE GND zone (not a multi-zone issue). NEXT (the actionable FR-on-Hub fix): get FR's autorouter to actually route the Hub's dense 4-layer core. Investigate, in order — (a) read the FULL freerouting.log (build/frlog/logs/) for why pass #1 makes only 1 change (clearance/keepout/layer-availability — is In2.Cu available to FR? is a rule-area keepout blocking it? is the netclass clearance from the committed .kicad_dru too tight for FR?); (b) try routing with the placement residual driven to 0 first (lever 3 above — overlapping courtyards/pads may make FR see no legal space); (c) compare the Hub DSN vs an eps DSN for what differs (layer `(type)` assignments, the `(rule (clearance ...))` block, `(keepout ...)`); (d) if FR fundamentally won't route a board this dense, fall back to the cec_route pcbnew toolkit (sub-agent routing pass) for the deterministic nets + GUI/Freerouting for the spine. The committed Hub (length 2359 / gates pass) is the free Tier-B calibration anchor once FR routes. An hour-long run is NOT worthwhile until this is fixed (it reproduces the 1-trace result).
  After 1–4, the Hub run IS the place→route→check validation against the fab board; MV3 similarity + the routed gates are the scorecard.

...
- [2026-06-16] **DONE — placement now resolves a body from prose** (was "finding must name a ref in proposed_lever.target"): implemented BOTH (a) the finder-prompt nudge (placement-arm guidance + _audit_prompt now tells the finder to put the refdes in proposed_lever.target) and (b) the prose fallback — `_prose_ref(finding, known_refs)` pulls a refdes (C1/U30/RS1) out of the `action`/`root_cause` prose and validates it against the live board's manifest refs (so RS485/INA240/CAN1 look-alikes can't masquerade), threaded as `known_refs` into `finding_to_delta` (replace-only). VALIDATED on the real Hub-smoke r2 finding: was `replace ref=None` → noop, now `replace ref=C1 status=applied` with a resolvable placement intent. scripts/cec_fs_actuator.py `_prose_ref`/`finding_to_delta`; scripts/cec_fullstack.py call site + _audit_prompt; tests/test_actuation_lever.py TestProseRefFallback (7).
- [2026-06-16] **r3-style lever-classification ambiguity: a free-form `action` sentence containing "waypoint" outranks "evict"/"shift"** — when proposed_lever has no structured `lever`/`type`, `_pl_lever` falls back to the whole `action` string and `_lever_kind` keyword-matches "waypoint" FIRST. The Hub r3 action ("shift component C1 ... +5VSB waypoint ... evicting any sensitive body") is clearly a placement move but classified as waypoint → noop (no geometry resolver), so prose-ref (replace-only) never runs on it. Fix candidates: (i) when both placement verbs (shift/evict/reposition) AND "waypoint" appear, prefer replace; or (ii) score keyword precedence by failure_class (placement → replace). Risk: changing precedence could regress legit "add waypoint intent" findings (r1) → wants its own check. Where: scripts/cec_fs_actuator.py `_lever_kind` ordering + `_pl_lever` whole-sentence fallback.

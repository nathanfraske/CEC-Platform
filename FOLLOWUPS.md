...
- [2026-06-15] **For placement to MOVE end-to-end, the finding must name a ref in proposed_lever.target** — the schema-tolerance fix (ff299ff) unblocks lever MAPPING (effort/replace now fire instead of noop), but the Hub r3 placement_eviction had target=None, and r5 named "C1" only in prose, so apply_placement_move can't resolve a body. Two options: (a) prompt the finder to put the ref in proposed_lever.target; (b) extract a ref token from the action/detail prose (regex for a refdes like C1/U30) in _pl_target as a fallback. Where: scripts/cec_fs_actuator.py _pl_target + the finder/auditor prompt (cec_fullstack.py _audit_prompt ALLOWED LEVERS). Until then placement deltas are created (replace) but resolve to no move on these boards.

...
- [2026-06-14] **MV5 generative closers (placer-upgrade) — measurement is in, generation is the next lever.** `hub_score` MEASURES `antenna` (ESP off its derived antenna edge) and `power_cluster` (mux-loop cohesion), and `proxy_score` applies SELECTION pressure (best-of-N prefers lower `hub_penalty`), but no pass yet GENERATES candidates that close them: the synth ESP lands central (antenna≈0.0) and the front-end smears (power_cluster low) because both are connectivity-placed and not biased to the WHY. Add when resuming the placer quality work (cec_synth_pipeline.py): (a) an **antenna-edge ESP seed** — bias the PCB-antenna IC's initial position in relative_place toward `cfg.params['antenna_edge']` (a board input; the WHY = the lobe radiates off an edge), keeping it movable; (b) a **power-loop cohesion sweep** — one barycentric pull on `build_hub_model().power_refs` (the mux+hold-up+LDO group) so the input loop tightens. Per the plan these are MV5's measurement-first design ("add the term, watch similarity rise, keep what helps"). The MV3 similarity number now measures the before/after so each closer is validatable. Resume: docs/placer-upgrade-2026-06-14/{plan.json,STATUS.md}.
- [2026-06-14] **Hub full-stack run (the owner's goal) — launch once the placer-upgrade audit lands.** With MV1–MV5 in, run `cec_synth_pipeline` place→route→check on `hubs/hub-standard` from a base stackup with `cfg.params['oracle_reference_path']='hubs/hub-standard/hub-standard.kicad_pcb'` (Stage-1 inputs auto-derived) and score the gap vs the fab board (HPWL/area/DRC/gates + MV3 similarity). The committed Hub is ALREADY routed (length 2359 / gates pass) so the Tier-B calibration anchor is free — do NOT build a routing campaign as a prereq (plan over-engineering warning). Owner pivot recorded in TODO; gated on the MV audit + (if it surfaces) the MV5 generative closers above.

...
- [2026-06-14] Placement Phase 2 — the corridor `cc=6` floor finding (docs/placement-strategy-2026-06-14.md CORE-PREMISE FINDING). Open items: (a) **the layer-assignment pivot** — the real fix for the pour-cut/~300C failure is route-time: assign each foreign net that crosses a FORMED corridor (the `high-current-corridor-keepout` checker now identifies them) to a non-pour layer, off the F.Cu/B.Cu pour. This is the owner's strategic call (vs. more placement tuning). (b) Phase 2 residual regressed 2→6 (the fixed spine + body-in-band veto over-constrain the legalizer; the committed board is residual 0) — tune the veto (it adds residual without lowering the inherent cc) and/or the legalizer; consider making the veto cover only LARGE bodies (a thin trace crossing is a layer problem, a body-in-band is the real placement fault). (c) Phase 2's `current_axis_offset`/`corridor_penetration` soft anneal terms (doc §3 Phase 2) were NOT added — the spine seed + veto subsume the formation; revisit only if a board needs the shunt off the seeded axis.

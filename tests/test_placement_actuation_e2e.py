...
class TestRealApplyPlacementMove(unittest.TestCase):
    ...
    def test_clean_board_resolves_no_corridor(self):
        # TEST-RIGOR-02: a CLEAN committed board (no body in a foreign band) -> verdict no_corridor
        out_rel = "build/fullstack/e2e-clean.kicad_pcb"
        if os.path.isfile(os.path.join(ROOT, out_rel)):
            os.remove(os.path.join(ROOT, out_rel))
        mv = fs.apply_placement_move(EPS_PCB, EPS_PCB, {"ref": None, "net": None},
                                     act.resolve_fence(), 990, out_rel=out_rel)
        self.assertEqual(mv.get("verdict"), "no_corridor", mv)
        self.assertFalse(os.path.isfile(os.path.join(ROOT, out_rel)))   # no override written

    def test_named_ref_not_in_violation_no_corridor(self):
        # TEST-RIGOR-02: a ref that exists but is not in any violation -> no_corridor
        mv = fs.apply_placement_move(EPS_PCB, EPS_PCB, {"ref": "U1", "net": None},
                                     act.resolve_fence(), 991, out_rel="build/fullstack/e2e-noref.kicad_pcb")
        self.assertEqual(mv.get("verdict"), "no_corridor", mv)

...
| REQ-MOD-COMMON-003 | EVERY enterprise module build SHALL carry **100BASE-T1 single-pair Ethernet on pair 2 (pins 4/5)** — bidirectional, replacing the Pro-tier RS-485 for the ENT line (owner ruling 2026-07-02 3rd; RS-485 remains the consumer Pro tier), EXTENDED TO THE 24-PIN by the 6th ruling: bandwidth is no longer the deciding criterion — the 24-pin gains the T1 attestation surface + a private switched channel on the fleet's most load-bearing validator (its rail data feeds REQ-HUB-COMMON-113 power-signature cross-checks of every OTHER module), native gPTP participation, fleet firmware-update/evidence-drain logistics off the shared CAN bus, and ONE uniform ENT MCU. DETECT = the locked 10 kΩ CAN+100BASE-T1 class on every ENT family, and every family SHALL participate in the fleet sub-µs time synchronization (REQ-HUB-COMMON-106). MCU: **ESP32-P4 uniformly across ALL FOUR ENT families** (survey 10: RMII + hardware 1588/PTP timestamps + PPS confirmed in ESP-IDF; reuses the already-paid 12VHPWR Pro QFN-104 reference design — one toolchain, one firmware base; STM32H563-class = documented price-competitive fallback ~$4.29–4.74; STM32G4 confirmed unable — no MAC-integrated 100BASE-T1 part exists; the 24-pin's earlier STM32G431 pick is SUPERSEDED by the 6th ruling). Module T1 front-end: DP83TC814S-Q1 default PHY ($2.39; TJA1103 $1.49 + 1588 is cheaper but NDA-flagged — clear with NXP first) + OPEN-Alliance CMC + AC coupling + PESD2ETH100 (~$3.0–4.2/module). | owner rulings 2026-07-02 (3rd, 6th); spec §2.3/§6.11; OQ-20 (ENT-resolved) | I+T | — |
...
| REQ-MOD-COMMON-013 | ADOPTED by owner ruling 2026-07-02 (6th) — pairs REQ-HUB-COMMON-114: ENT modules SHALL implement the **pin-7 heartbeat responder**: edge capture + open-drain drive on pin 7; the response is a hardware-timed edge/pulse pattern derived from the module device key + the per-challenge nonce, by the signed-firmware-prescribed method. Implementation contract: the nonce rides CAN/T1 ahead of the timing window (compute-then-respond — crypto compute time never sits in the timed path); the response edge is scheduled by hardware timer compare (ESP32-P4 timer+ETM/output-compare class, per the uniform ENT MCU in REQ-MOD-COMMON-003), so response determinism is a timer-peripheral property, not a firmware-loop property. Graceful degrade BOTH directions (locked §1/§8 preserved): on a Hub that never challenges (Standard/Pro/consumer), the responder stays dormant and the module functions normally; a legacy module (pin 7 NC) is never challenged — the REQ-HUB-COMMON-114 auto-untrust policy binds only to modules whose attested class claims pin-7 capability. ≈$0 parts (GPIO + existing timer peripheral). | owner ruling 2026-07-02 (6th); REQ-HUB-COMMON-114; REQ-MOD-COMMON-010/012 | T | — (Phase-4 SATISFIED: spec v1.2.0 applied 2026-07-02) |
...
| ID | Requirement | Trace | Verify | Gate |
|---|---|---|---|---|
| REQ-MOD-AIR-020 | ENT-AIR module builds SHALL use radio-free MCUs — option (a) RESOLVED by owner ruling 2026-07-02. Working baseline: **ESP32-P4 (radio-free) uniformly across all ENT module families** per the T1 rulings (3rd + 6th — the earlier STM32G4 split baseline [G431 digital families / G474 12VHPWR-Std] is SUPERSEDED; STM32G4/H5-class remains the documented radio-free fallback). The fused-off/antenna-absent ESP32 posture is STRUCK: no Wi-Fi-disable eFuse exists on S3/C6, no radio-absent SKU exists, and it fails inspection-without-powering (survey 8). | plan §1a.5; survey 8; owner ruling 2026-07-02 | I | — |
...
| REQ-MOD-COMMON-031 | Each family SHALL ship an FMEA/FMEDA covering the in-path elements (shunts, connectors, PCB copper) with fault-injection test evidence, plus the §6.6/§6.7 thermal/vertical-transition analyses at rated current. | audit §1.4; spec §6.6/§6.7; OQ-10/12 | A+T | OQ-10/12 |

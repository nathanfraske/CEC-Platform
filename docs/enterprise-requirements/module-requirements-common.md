...
| REQ-MOD-COMMON-053 | Enterprise module RJ-45 jacks SHALL fail safe against the same mis-plug set as REQ-HUB-COMMON-110 (live switch cable / 57 V passive PoE, both modes and polarities): no hardware damage, self-recovering. Module-side exposure differs from the hub: pin 1 is the module's 5VSB INPUT (57 V would reach the module LDO — needs OVP/blocking rated ≥60 V), DETECT carries the module's code resistor (series element ahead of it), CAN pins covered by the TJA1051T/3. Verified by injection test per family. | owner ruling 2026-07-02 (4th); REQ-HUB-COMMON-110 | T | — |

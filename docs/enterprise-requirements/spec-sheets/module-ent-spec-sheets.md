...
| Per-unit identity | **RESOLVED (5th ruling)**: MCU-resident device key + Hub challenge-response over CAN/T1 + DETECT poke-and-ack liveness — NO identity hardware (1-Wire path OUT); provisioning = key injection at the flashing step. PROPOSED addition (owner exploration, adopt/decline pending): **pin-7 heartbeat responder** (REQ-MOD-COMMON-013) — port-bound hardware-timed challenge-response; miss → auto-untrust at the Hub | **≈$0 parts** (firmware + provisioning step; heartbeat = GPIO + existing timer) | REQ-MOD-COMMON-010/011/013; OQ-76 resolved |

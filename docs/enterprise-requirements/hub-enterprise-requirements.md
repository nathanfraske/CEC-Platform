...
| REQ-HUB-COMMON-043 | The Hub SHALL service the module streaming pair (pins 4/5) on all 8 ports as **100BASE-T1 only** — RS-485 backward compat is DROPPED (survey 10 recommendation, owner-review tag): no vendor precedent exists for bridging a live T1 PHY MDI to an RS-485 transceiver on a shared pair, and the platform's own §8 pattern already covers it (a consumer Pro module's streaming pair goes dark on an ENT port exactly as it does on a Standard Hub today; CAN control+telemetry stay fully serviced, REQ-045 unaffected; saves $8.93–29.93/hub vs mux dual-mode). Hub-side termination: **2× Microchip LAN9370** (4-port 100BASE-T1 switch, integrated PHYs + 802.1AS/1588v2 HARDWARE timestamping) bridged to the PolarFire fabric via 2 RGMII/MII MAC instances (~5% LE) — supersedes the 8-discrete-PHY/soft-switch and THVD1450 receiver-bank designs. LAN9374 (6×T1) is the one-chip fallback if port count drops to 6. | survey 10; owner ruling 2026-07-02 (3rd); OQ-20 (ENT-resolved) | T | — |
...
| REQ-HUB-COMMON-110 | Every module port SHALL FAIL SAFE against a live network cable mis-plug (owner ruling 2026-07-02 4th): the port SHALL withstand, WITHOUT hardware damage and self-recovering on removal (no sacrificial elements requiring field repair), (a) standard IEEE 802.3 10/100/1000BASE-T signaling from a switch, and (b) worst case 57 V PoE — both 802.3af/at/bt modes, both polarities, INCLUDING always-on passive injectors (no detection handshake). A resettable element or auto-retry active limiter satisfies self-recovery; a one-time/non-resettable fuse does NOT (survey 11). The mis-plug SHALL be detected, alarmed, and logged (anomalous DETECT reading / no valid module class). Rationale delta vs the consumer §2.4 ratification: ENT-NET carries a real Ethernet jack on the same faceplate, making the mis-plug foreseeable misuse, not deliberate. Per-pin analysis baseline: CAN pins covered by TJA1051T/3 ±58 V; DETECT needs a series element ahead of the ESD clamp; VCC pin 1 needs 60 V-class blocking; pin 7 needs a defined ≥60 V-tolerant termination; T1 pair per the PHY's fault rating (survey 11). Containment: a mis-plugged port SHALL NOT disturb the other ports, the shared CAN bus, or hub operation (hub-level fail-operational; the fault is port-local), and the port SHALL return to service on cable removal with no intervention. Fail-functional THROUGH an energized PoE cable is explicitly out of scope — the DC-coupled link (CAN ±12 V operating common-mode, DETECT, 5VSB) cannot signal under 48–57 V common-mode; documentation SHALL state this. Verified by an actual injection TEST at both hub and module ends. | owner ruling 2026-07-02 (4th); spec §2.4 re-scope; survey 2 confusion-proofing | T | — |
| REQ-HUB-COMMON-112 | ADOPTED by owner ruling 2026-07-02 (5th) — the formal locked-table change rides the v1.2.0 spec edit: pin 7 (the reserved spare) SHALL be allocated as the ENT **hardware SYNC/FREEZE line** — a shared wired-OR edge across all module ports giving (a) a platform-wide simultaneous FREEZE trigger and (b) a PPS-class latch edge complementing PTP. Target alignment: ≤100 ns module-to-module (≈5 ns/m propagation + driver/receiver skew; sub-ns is NOT claimed — White-Rabbit-class, no identified need vs the 200–500 ns Max-tier arc band). This edge SHALL also be the REQ-106 bench-verification mechanism. Suitor conflict CLEARED by the same ruling: 1-Wire identity is OUT (poke-and-ack + link challenge instead, REQ-MOD-COMMON-010) and the DETECT Kelvin return is OUT — pin 7 is uncontested. Remaining mechanics: survey-11 protection re-scope (driven line, PoE Alt-B pairs 7/8) and backward compat (legacy modules NC — harmless; ENT modules enable sync only after the CAN handshake confirms an ENT hub). Subsumes the OQ-60 companion-connector FREEZE-trigger role for the general module fleet. | owner ruling 2026-07-02 (5th); spec §2.3 pin table; OQ-60 | A+T | Phase-4 spec edit |
| REQ-HUB-COMMON-113 | Because a module's self-reported validation is inherently untrusted, the Hub SHALL cross-validate each module across INDEPENDENT surfaces — at minimum two of: DETECT analog class + poke-and-ack liveness, CAN challenge-response against the module device key, T1-link attestation/behavioral checks, and power-signature consistency (the module's own draw as seen by the 24-pin/rails) — and SHALL alarm + log any cross-surface inconsistency (identity mismatch, class-vs-claim drift, liveness failure). Cross-dimensional consolidation of these surfaces is an ENT analytics feature, not a module self-claim. | owner ruling 2026-07-02 (5th, cross-validation rationale); REQ-MOD-COMMON-010/011; tamper §5 | T | — |
| REQ-HUB-NET-111 | PROPOSED (owner exploration 2026-07-02, adopt/decline pending): the ENT-NET uplink jack MAY additionally be an 802.3af/at PD — a galvanically isolated PD front-end (PD controller + isolated converter behind PoE-capable magnetics) feeding a FOURTH slot of the §2.9 priority-OR behind its own eFuse front (lowest priority / forensic-policy-gated), segregated and untrusted like the external feed. Class mapping: af (~13 W) SHALL cover the STANDBY posture (the forensic case); at (~25 W) MAY cover FULL. ENT-AIR unaffected (no jack). If adopted: re-verify the uplink OQ-14 protection topology WITH the PD present (the absent-PD-signature rejection argument inverts), and the PD path SHALL appear in the REQ-050 per-source monitoring. Adder ~$5–10 [est]. | owner exploration 2026-07-02; spec §2.9; survey 2 | A+T | D-ENT-5 |
...
| REQ-HUB-COMMON-106 | The module-facing 100BASE-T1 links SHALL provide fleet-wide TIME SYNCHRONIZATION to sub-microsecond accuracy (gPTP with hardware timestamping at every stage — LAN9370 on-chip 802.1AS/1588v2, module MAC-level timestamps + PPS; 802.1AS's own <1 µs/≤7-hop target vs our 1–2 hops; industry ladder puts HW-timestamped gPTP at 50 ns–1 µs class), so multi-module captures are correlatable at sub-µs. This is a DESIGN TARGET pending bench measurement — no vendor publishes a topology-exact figure. Documentation SHALL state that sub-µs applies to SYNC, not message latency (frame time 6.72 µs at 100 Mb/s, verified — the nanosecond FREEZE path remains the OQ-60 hardware-trigger proposal, out of scope here). | survey 10; owner ruling 2026-07-02 (3rd) | T | — |

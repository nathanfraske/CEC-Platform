...
| REQ-HUB-COMMON-112 | PROPOSED (owner exploration 2026-07-02, adopt/decline pending — LOCKED-TABLE change, owner's pen at v1.2.0): pin 7 (the reserved spare) MAY be allocated as the ENT **hardware SYNC/FREEZE line** — a shared wired-OR edge across all module ports giving (a) a platform-wide simultaneous FREEZE trigger and (b) a PPS-class latch edge complementing PTP. Target alignment: ≤100 ns module-to-module (≈5 ns/m propagation + driver/receiver skew; sub-ns is NOT claimed — White-Rabbit-class, no identified need vs the 200–500 ns Max-tier arc band). This edge SHALL also be the REQ-106 bench-verification mechanism. Conflicts to resolve at adoption: pin 7's other suitors (OQ-76 1-Wire identity return — can use GND instead; OQ-60 DETECT Kelvin return), survey-11 protection re-scope (driven line, PoE Alt-B pairs 7/8), and backward compat (legacy modules NC on pin 7 — harmless; ENT modules enable sync only after the CAN handshake confirms an ENT hub). Subsumes the OQ-60 companion-connector FREEZE-trigger role for the general module fleet. | owner exploration 2026-07-02; spec §2.3 pin table; OQ-60/76 | A+T | D-ENT-5 |

...
| # | Decision | State |
|---|---|---|
| D-5/D-5a | 24-pin beta scope + **output form**. Form menu now: owner lean (very short stub + order-time optional extension), **Option F perpendicular daughtercard** (vertical female on an edge-soldered card — machine-solderable/AOI-able, solves the panel's own top objection to hand-crimped pigtails; needs the owner's vertical-female MPN from his hunt + the mating-force anchor design), Form B (12cm stub), incumbent C. NEW VERIFIED FACT strengthening the move off C: **no F-F 24-pin cable exists as a commodity product anywhere** (only DIY housings/terminals) — the incumbent always meant CEC fabricates a cable class with zero commercial supply chain. | OPEN, owner lean recorded |

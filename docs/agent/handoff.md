# Agent handoff — 2026-07-08 20:40:09Z
...
HEAD:    7942df1 wave: LOCAL INTENT-PROPOSER SEAT between waves (owner GO -- agents drive the levers)
status:  19 uncommitted path(s)
```

### Recent commits
```
7942df1 wave: LOCAL INTENT-PROPOSER SEAT between waves (owner GO -- agents drive the levers)
e834ecd meta: G8 state (round-2 queue landed + speedup pass) -- FOLLOWUPS/TODO/cost-policy
24d1461 oracle+placer: GAP-PROFILE advisory + ROLE-KEEPOUT lever, inert (round-2 item 7, teeth)
a1fefe4 wave: PARALLEL candidate loop + lazy silk + THERMAL MIRAGE GUARD (profiling pass)
3acbe5f oracle: LAZY THERMAL -- solve only would-be gate-clean candidates (owner directive)
223b95a fiducials: EMIT at materialize + QUALITY gate (round-2 item 6, teeth incl. sabotage)
1e90823 oracle: FACING-FRACTION advisory metric (round-2 item 4, drives the backlogged face() lever)
a8c0c8b oracle: SILK score/fp = the tier-0 craft tie-break (round-2 item 3, teeth-verified)
5c8e231 oracle: ROUTE-SANITY advisory (detour ratio + via budget, round-2 item 5, teeth on synthetics)
411c0c9 oracle: PIN-ESCAPE + COURTYARD-EDGE gates (exploration round 2 implement queue, teeth-verified)
d73c1a3 meta: lever-pass state (exploration round 2 verdicts + implement queue)
4948233 oracle: orphaned pour-family + DFM checkers wired as ADVISORY (calibration owner-queued)
```
...
## Broker
{"running": {"cec-manager-fast": {"since": 1783542738, "active": 0, "idle_s": 273}, "cec-worker-quality": {"since": 1783543099, "active": 0, "idle_s": 95}}, "gpu_budget_gb": 30.0, "gpu_used_gb": 27, "idle_stop_s": 1800.0, "stats": {"requests": 3087, "starts": 100, "evictions": 65, "by_client": {"fs-warm": 250, "CEC-Platform": 2624, "vlm-bakeoff": 38, "probe": 1, "capstone-smoke": 1, "v4-task": 5, "seat-bakeoff": 70, "seat-bakeoff-probe": 3, "cec-inventory-extractor": 7, "cec-inventory-keepwarm": 1}}}

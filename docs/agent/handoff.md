# Agent handoff — 2026-07-25 17:01:38Z
...
HEAD:    9b963b41 hub unblocked: pin SWIG's type registry (the all-9999 wall was never seat pressure)
...
e08c4f60 pour-first stage fail-closed on the grade side (owner shippability ruling)
ee0ee191 pour-termination + via-in-pad rulings: pad-edge clip, gap exclusion, assembly-class via guard
72e2c2f8 via-in-pad ruling: assembly-class own-net pad exclusion for every via path
```

### Working-tree changes
```
 M scripts/cec_fresh_wave.py
?? beta/12vhpwr-standard/candidate/
?? beta/atx-24pin-rev3/candidate/
?? beta/hub-standard-rev2/candidate/
?? scripts/cec_candidate_sync.py
?? tests/test_candidate_ref.py
```

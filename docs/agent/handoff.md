# Agent handoff — 2026-07-08 03:45:16Z
...
HEAD:    cf6bfec wave: back-face renders get an on-image MIRRORED-VIEW banner (owner confusion fix)
status:  21 uncommitted path(s)
```

### Recent commits
```
cf6bfec wave: back-face renders get an on-image MIRRORED-VIEW banner (owner confusion fix)
a5aa794 24pin: hub-jack edge coupling (owner directives)
dd0f96e 24pin: owner batch -- 1x4 stub (study item 5 CLOSED), J3 mouth-out, buttons cluster, stub alignment, size 70x55
9153f5f wave: per-variant REVIEW SNAPSHOTS on the dash feed (owner ask)
241eb01 wave: per-board FR effort/timeout (the 24-pin timeout diagnosis)
88939b2 oracle: sense-side gate term -- analog never crosses faces, independently verified
2e3fccd cec_fr: per-SIDE kelvin taps/pours/keepouts + straddle-aware pair defaults (the 24-pin wave unblocked)
0fefd85 24pin: placement_directives block in the board manifest = the canonical ruleset home
778a8b6 dual-sided: HARD GUARD -- connectors and the MCU never go on the back (owner rule)
33b0a2a meta: mechanism package state (dual-sided landed; per-side taps/pours remain)
84e9709 placer: DUAL-SIDED rail chains live end-to-end (24-pin mechanism item e)
45235fe place(flip): REAL-PART back-side placement, calibrated against pcbnew native Flip()
```

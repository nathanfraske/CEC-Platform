# Agent handoff — 2026-07-15 19:57:58Z
...
HEAD:    2b4fe1a hub-standard-rev2: new revision schematic = beta donor + J6 mezzanine
status:  6 uncommitted path(s)
```

### Recent commits
```
2b4fe1a hub-standard-rev2: new revision schematic = beta donor + J6 mezzanine
d164266 plateau-kill stream: merge stderr + select deadline (wave-13 stall fix)
af072ac GND island stitch + J2-near-lane-6 + the plane forensic (both layers)
d0ba984 FOLLOWUPS: RJ-45 overhang/clip numbers + J2-near-lane-6 analysis
b2c6b55 plateau-kill signal: togo+failed both-flat (wave-11 calibration)
6ea9970 route_tiered cured (keepouts + refuse-loud gate); external plateau-kill
4378404 FR default -> cec2; noecho+progress validated; maxstall experimental
3c0a9c3 fixup: broker cec_agent.py belongs in the existing vendored ops/cec-llm-broker (provision deploys from there); drop the redundant ops/broker
33e286b ops/broker: durable cec_agent.py copy (broker dir is not a git repo)
6a2153e CEC compute tools for agents (MCP + broker CLI) + cec2 fork docs/patch
8486da5 FR default -> 1.7.0-cec1 fork; seed axis opt-in (CEC_FR_SEED_AXIS)
4da1d00 wave: precision branch OFF for 12vhpwr (measured cell-bulldozer mitigation)
```

### Working-tree changes
```
 M .claude/memory/current-work-handoff.md
 M .claude/memory/lesson-ablation-before-narrative.md
 M FOLLOWUPS.md
 M TODO.md
...
## Broker
{"running": {"cec-worker-quality": {"since": 1784145077, "active": 0, "idle_s": 389}}, "gpu_budget_gb": 30.0, "gpu_used_gb": 19, "idle_stop_s": 1800.0, "stats": {"requests": 76, "starts": 11, "evictions": 6, "by_client": {"CEC-Platform": 10}}}

# Agent handoff — 2026-07-07 23:52:59Z
...
HEAD:    45235fe place(flip): REAL-PART back-side placement, calibrated against pcbnew native Flip()
...
a3ab56a kelvin pairs: SHUNT-STRADDLE derivation (net-name-independent) in both derivation sites
652eff7 TODO: 24-pin mechanism package extended with owner directives (dual-sided/antenna)
7e18167 placer: rounded-corner outlines + mounts-none + NPTH pads never in the overhang band (24-pin owner directives)
b690091 TODO: 24-pin board-design mechanism package from the probe
d4bfbca 24pin-rev3: RS4 reverted to the AS-BUILT 2-terminal 5VSB shunt (owner correction)
69d9955 dashboard: ACTIVITY feed -- every piece of agent work visible with its artifacts (owner ask)
f97dce1 24pin-rev3: margin bank re-homed to its own MARGIN RESERVOIRS section + README TB correction
a7ae6a3 memory: handoff thread (G) -- 24-pin ground-up remake passes 1+2
af3a669 meta: 24-pin remake pass-2 bookkeeping (owner-queue resolutions + followups)
```

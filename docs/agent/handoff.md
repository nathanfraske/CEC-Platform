# Agent handoff — 2026-07-15 23:50:35Z
...
HEAD:    8e0ba8c3 relocate pool repro to scripts/ (build/ is gitignored)
status:  5 uncommitted path(s)
```

### Recent commits
```
8e0ba8c3 relocate pool repro to scripts/ (build/ is gitignored)
c2f2643b hub MCU edge seat: WROOM NoAntKeepout variant (beta W9) + pool-bug closure
629440d1 p8b affinity re-seat: respect the pass-lock registry (wave-5 LockViolations)
4ac1b0e8 hub wave-5 rungs: affinity re-seat, WROOM edge seat, width visibility
f0d6523d hub-rev2 wave-4 close: winner unconn 11 / safety gates pass; trackers
3525480d supercap FINAL: DNP 2S radial provision, hand-solder this run (owner)
06093f28 supercap re-size: 5-10s target, LCSC-native 2S small cells (owner)
eca7f22a supercap ruling: Pro=b2 (PHV 2.5F), Max=b1 (PHB 5F) — owner split
90c360f5 supercap study §7: owner decision table (no-IC Pro/Max shapes, ENT d-vs-e)
9fa50094 supercap queue row refined: Pro/Max = no-IC passive shape (owner steering); manager = ENT only
66492afb supercap UPS study (Sonnet-delegated) + owner-queue decision row
579a10da hub-rev2: per-port RJ45-VCC polyfuses (Sonnet-agent splice, audited)
```

# Agent handoff — 2026-07-07 21:08:15Z
...
HEAD:    90ebed4 dashboard: stale-tab self-reload (page rev stamp) + no-store + per-row panel counts
status:  14 uncommitted path(s)
```

### Recent commits
```
90ebed4 dashboard: stale-tab self-reload (page rev stamp) + no-store + per-row panel counts
a96f40b FOLLOWUPS: eps envelope-thermal gap + oracle gate-current decision
6bd86ac dashboard: generic EXTRA study panels on archive entries
e6013d8 dashboard: RENDER + copper PLOT panels in the analyzer (owner ask)
c1474be memory+TODO: wave-1 eps GATE=TRUE result recorded
12340da score: geometry-VERIFIED waivers for two documented headless DRC artifact classes
c762782 FOLLOWUPS: test re-baseline + wave-1 thermal re-grade + blade contact-R
745addc thermal2d: blade fields are LO/output sinks in _default_src_sink
3db327f memory: handoff thread (F) -- pipeline consolidation + fresh-board wave state
2430ec7 wave driver + dashboard explorer + tap/field refinements
aa1d402 placer+taps: D-5a blade-field support + bent-tap fallback (clears the INA238 LO-tap blocker)
7d281a9 pipeline: resolve the ROOT schematic of hierarchical beta boards (find_root_sch)
```

### Working-tree changes
```
 M tests/test_corridor_keepout.py
 M tests/test_corridor_model.py
 M tests/test_foreign_on_pour_gate.py
 M tests/test_placement_actuation_e2e.py
 M tests/test_placement_session.py
 M tests/test_placer_oracle.py
...
## Broker
{"running": {}, "gpu_budget_gb": 30.0, "gpu_used_gb": 0, "idle_stop_s": 1800.0, "stats": {"requests": 3078, "starts": 95, "evictions": 63, "by_client": {"fs-warm": 250, "CEC-Platform": 2619, "vlm-bakeoff": 38, "probe": 1, "capstone-smoke": 1, "v4-task": 5, "seat-bakeoff": 70, "seat-bakeoff-probe": 3, "cec-inventory-extractor": 7, "cec-inventory-keepwarm": 1}}}

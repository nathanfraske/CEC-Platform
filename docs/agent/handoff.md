# Agent handoff — 2026-06-12 19:25:12Z

_Auto-generated by .claude/hooks/session-end.sh and pushed to `ops/agent-handoff`._
_Per the WSL-ephemeral state policy: this is the durable, remote copy._

## Repo state
```
branch:  claude/wsl-ephemeral-recovery
HEAD:    bf11d7e ops: WSL-ephemeral state policy + one-shot recovery (provision.sh, handoff hook)
status:  0 uncommitted path(s)
```

### Recent commits
```
bf11d7e ops: WSL-ephemeral state policy + one-shot recovery (provision.sh, handoff hook)
cf950cc Merge pull request #48 from nathanfraske/claude/sb08-item3a-synth-copper
f3ba793 Merge pull request #43 from nathanfraske/claude/sb08-fr-variance-report
e56dc30 Merge pull request #47 from nathanfraske/claude/ai-box-upgrade-analysis
54b3998 Merge pull request #46 from nathanfraske/claude/refute-reason-class-counter
87d2023 Merge pull request #45 from nathanfraske/claude/zone-progression-plot
13ad9b8 corpus: gate-redefinition reruns the packet fixture; round 4 must still fail (owner, PR #43)
5dfa220 Merge pull request #44 from nathanfraske/claude/sb08-item5-golden-stability
2dc3bd3 SB-08 item 2: pour-integrity F+B-mirror-aware -> 3a full gate stack green (75.5C)
374f362 Merge pull request #40 from nathanfraske/claude/det-inspection-pipeline
8030fef SB-08 item 3b (PROPOSAL fallback): thermal acceptance rationale for owner signature
1fa952c corpus: 3 SB-08 band-discipline candidates staged (owner items 12-14)
```

### Working-tree changes
```
```

## Broker
{"running": {}, "gpu_budget_gb": 30.0, "gpu_used_gb": 0, "idle_stop_s": 1800.0, "stats": {"requests": 0, "starts": 0, "evictions": 0, "by_client": {}}}
## Persistent memory snapshot
### current-work-handoff.md

# Current work handoff

_Updated 2026-06-12 by the env-rebuild session._

## Context
The WSL distro was reinstalled fresh on 2026-06-12 after a failed move to E:. The
Linux home (repos, toolchains, the cec-llm-broker, persistent memory) was wiped; the
GGUF models on `/mnt/e/AI Models` survived. This session rebuilt the whole environment
from scratch. See [[env-rebuild-2026-06-12]] and [[llm-broker]].

## Done + verified this session
- Re-cloned `nathanfraske/CEC-Platform` → `/home/nathan/CEC-Platform`.
- Python deps (apt, system python3): numpy/matplotlib/PIL/scipy — import OK with pcbnew.
- KiCad 10.0.3 (kicad-cli + pcbnew), Java 21 full JRE + xvfb.
- Docker 29.5 + NVIDIA Container Toolkit; GPU passthrough into a container verified (RTX 5090).
- Routing image `cec/routing:kicad10` built.
- `cec-llm-broker` rebuilt at `/home/nathan/cec-llm-broker`, installed as a systemd unit,
  catalog/health/stats endpoints verified, compose orchestration wiring cross-checked.
- llama.cpp `server-cuda` image pre-pulled.

## Verified end-to-end
- Real broker boot of `cec-worker-quality` round-tripped: request → on-demand `compose up`
  → GPU cold-load (~165 s) → proxied llama.cpp `chat.completion`. GPU then returned to
  baseline after stop. The full toolchain rebuild is COMPLETE and the LLM stack is live.

## Notes / next
- Not provisioned (out of scope / weren't present before): `deepseek-v4-flash` GGUF, the
  vLLM `cec-judge` HF model (auto-downloads on first request), the sibling `cec-runs` repo
  (clone next to CEC-Platform when ledger writes are needed — compose mounts `../../cec-runs`).
- The previous session's own current-work-handoff was lost in the wipe and could not be
  recovered; project state should be re-derived from git history + docs/owner-queue.md.

### MEMORY.md (index)

# CEC-Platform memory index

- [Environment rebuild 2026-06-12](env-rebuild-2026-06-12.md) — WSL reinstalled; full toolchain + broker rebuilt from scratch.
- [LLM broker](llm-broker.md) — cec-llm-broker at /home/nathan/cec-llm-broker, :8080, systemd unit; orchestrates the compose LLM backends.

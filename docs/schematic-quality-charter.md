...
| T1 | `scripts/cec_sch_layout.py` | Rotation-aware placement (round-trip-verified pin math); `wire_adjacent` real Manhattan wires for local pairs; `place_decouplers`/`wire_decouplers` (netlist-derived IC ownership via `derive_owners`, wired not labeled — ports cec_pcb's auto_cluster concept); text-collision `text_bbox`/`detect_overlaps`/`nudge_texts` (calibrated against a real `kicad-cli sch export svg` textLength measurement — Newstroke runs ~1.02x font-size/char, not the ~0.75x a monospace guess would give); `--check-overlaps`/`--nudge`/`--demo` CLI. Rotation convention (standard CCW on Y-up local coords, applied before the Y-flip) empirically validated by round-tripping a rotated R_Small through `kicad-cli sch export netlist` at 0/90/180/270 (a sign error swaps pin 1/2 — a real discriminating test, not a tautology). Teeth: `tests/test_sch_layout.py` (12/12 — rotation round-trip all 4 angles, wire_adjacent close/far, decoupler adjacency + derive_owners balancing/signal-coupling, font calibration vs measured SVG, overlap detect+nudge, rotated-field bbox semantics, and the eps-8pin real-collision check: SENSEC*/VBUS labels colliding with GND ports — evidence for the module's whole thesis, that board is NOT touched). `build_demo()` produces `build/sch-layout-demo.kicad_sch` (IC + 4 wired decouplers + a real-wired divider chain + tapped filter cap), ERC-error-clean and overlap-free after its own nudge pass. GOTCHA recorded in the module: a `PWR_FLAG` INSTANCE alone is not enough — its lib_symbols definition must also be embedded, or KiCad silently reports the pin "Unspecified" and the flag never satisfies ERC's driven-pin check. SECOND measured gotcha (2026-07-03, found integrating into hub-enterprise): KiCad renders a symbol FIELD at (symbol rotation + stored field angle) — a rot-90 part's field at stored angle 0 draws vertical (`rotate(-90)` in the SVG); `_extract_text_elements` now computes property bboxes at the RENDERED angle via the parent symbol's rotation (before the fix, a rot-90 inductor's 21-char Value was bboxed as a tall strip, false-collided, and got nudged 40 mm off its part) | **LANDED 2026-07-03; INTEGRATED into `hubs/hub-enterprise/gen_hub_enterprise.py` 2026-07-03** — all 7 leaf sheets 01a–01g composed (decouplers/dividers/pin-buses as drawn wires, per-sheet centering, A4 leaves) + the thin parent (real eFuse→cascade inter-box wires, sheet-pin names inside boxes, grid-aligned box geometry killing the whole `endpoint_off_grid` ERC class 19→0), netlist-identity proven node-for-node (46 groups), `nudge_texts` runs as the generator's finishing pass, overlaps 45→0 across all 8 generated sheets |
...
| T4 | Composition engine (extends T1) | Sheet archetypes + flow: netlist-DAG left-to-right ordering (sources left, loads right, rails top, GND bottom); per-block templates (regulator, transceiver, connector, sensor chain); pin-side selection + net ordering to MINIMIZE crossings; bus notation for grouped signals (RGMII/QSPI/SDIO). **Composition rules of record: `docs/schematic-composition-standard.md`** (Nuand-derived, owner-ruled 2026-07-02/03: edge-anchored I/O columns, flow baselines, captions/notes, uniform banks, ref/value convention, pin-glyph clearance, PWR_FLAG block) — update it in the SAME change as any engine/archetype change | IN PROGRESS — `cec_sch_compose.py`+`cec_sch_archetypes.py` promoted 2026-07-03; standard-doc conformance pass (io columns, baselines, captions, pin-glyph-aware overlap gate) landed on hub-enterprise 01a–01g + ent-common |
...
The ENT hub per-sheet gate (SCHEMATIC-PLAN.md §2) grew two legs when T1–T3 landed —
**ADOPTED 2026-07-03** (both green on the regenerated 01a–01g + thin parent):
5. `cec_sch_layout.py --check-overlaps` ≤ threshold (target 0) — currently 0 on all 8
   generated sheets.
6. `cec_sch_lint.py` clean (style classes triaged like ERC: real vs documented-benign) —
   ERROR 38→0 (the parent's off-grid box-edge stubs, fixed by grid-aligned box geometry),
   WARN 57→15 (all SL-04 label-angle-vs-wire cosmetics at L-wire corners).
Sheet 02 additionally gates on the T2 pin-type audit of `cec-ent-compute`/all cec-ent libs.

## Standing integration order

T1+T2+T3 land → ~~integrate T1 into `hubs/hub-enterprise/gen_hub_enterprise.py`
(regenerate 01a-g, netlist-identity proven)~~ **DONE 2026-07-03** (owner's centered/
compact/wired-decouplers correction; 59-component/46-group node-set identity proven,

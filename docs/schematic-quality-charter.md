...
| T4 | Composition engine (extends T1) | Sheet archetypes + flow: netlist-DAG left-to-right ordering (sources left, loads right, rails top, GND bottom); per-block templates (regulator, transceiver, connector, sensor chain); pin-side selection + net ordering to MINIMIZE crossings; bus notation for grouped signals (RGMII/QSPI/SDIO) | SCOPED — T1 is integrated (2026-07-03), so T4 starts with the next-captured sheets (02+); the 01a–01g compositions (eFuse template ×3, cascade, holdup, buck, rail-sense) + the thin parent's lane-routed no-crossing fan-in are its hand-built precursors |

...
- **Parts pricing method:** every line traces to the board's committed BOM CSV (reference,
  ...
  Digi-Key/Mouser market bands for the connector class and flagged **UNVERIFIED — reverify at
  BOM lock**; live quotes were not reachable this pass.
- **PCB fab:** 4-layer FR4 at the measured board sizes (`docs/standard-tier-review/routing-foundation-2026-07-04.md`
  §1 stackup register + this pass's own `pcbnew`-free bounding-box read of each committed
  `.kicad_pcb`), 2oz outer per the cable-board convention (24-pin is 1oz outer, a flagged platform
  deviation — routing-foundation §1). Fab $ are **estimated from typical JLCPCB 4-layer small-board
  pricing bands** (a live quote needs uploaded gerbers, not reachable this pass) — flagged UNVERIFIED.
...
the straight TE 63849-1 (C86469, priced in `output-daughterboard-study-2026-07-04.md` §8.10 at
"from $0.0405") to the **right-angle TE 63951-1 (C591344)** — consistent with the boards' own
"stands perpendicular, tabs exit the board face" posture ruling. Joint counts are unchanged
(9/6-per-board/4-per-board). C591344 has no cached price in this repo; same TE FASTON tab family/
class is assumed (~$0.045@100/$0.035@1k each) but this is **UNVERIFIED — reverify C591344
specifically**, since a right-angle stamped tab can price a few cents above the straight part this
repo priced. ATX24-db (81×17mm, 9 tabs) ≈$1.05@100/$0.72@1k; EPS-db (53×15mm, 6 tabs, ×2/module)
≈$0.72@100/$0.51@1k each; PCIe-db (35×15mm, 4 tabs, ×2-3/module) ≈$0.63@100/$0.44@1k each. No
active/passive components on any daughterboard (BOM-confirmed).
...
sourcing passes and the v1.4.0 daughterboard architecture. Two of those targets have already been
measured stale by the platform's own review docs, independent of this study: Hub's recorded
...
| 12VHPWR Standard | $28.05 | $98.2 | **$99** |
| Hub Pro (est.) | $21.46 | $75.1 | **$79** |
| 12VHPWR Pro | $90.21 | $315.7 | **$299** (3.31×, leaves headroom under Max's $499-599) |
| EPS-Pro / PCIe-Pro (bounded) | ~$75-98 | ~$263-343 | **$269-349 band, unpriced/proposed** |

**3.2 PMD2 sanity anchor.** The only per-connector power-logging competitor found,
ElmorLabs PMD2, sells at **$99** as a self-contained single-connector logger with a slow-logger
"scope" mode (`docs/research/max-instrument-channel-decision-2026-06-11.md` §1, [I-6]). 12VHPWR
Standard landing at $99 is a direct, honest comparison point: **same price, but 12VHPWR-Std is one
rail of a networked, always-on, CAN-bus-connected system**, not a standalone dongle you plug in to
take a reading. The fairer whole-system comparison is Hub+24-pin ($49+$89=$138 à la carte, or the
$119 base bundle, §4a) against PMD2: for roughly the same money as PMD2 plus a modest premium, the
customer gets four continuously-monitored rails (12V/5V/3.3V/5VSB) instead of PMD2's one, plus a
CAN-networked platform that further modules plug into at a much lower marginal price than $99 each
(because the MCU/host-USB/enclosure cost is already sunk in the Hub). That marginal-module economy
is the core bundle argument in §4.
...
| 12VHPWR Standard | $28.05 | $21.02 | $99 | $84 (loaded) |
| **(c) LOADED bundle — base bundle + EPS + PCIe-2 + 12VHPWR-Std + 3 more patch cables ($15)** | ~$118 | ~$88 | $147+69+69+99+15=**$399** sum | **$339 (-15%)** |
| Hub Pro (est.) | $21.46 | $15.80 | $79 | $71 (Pro bundle) |
| 12VHPWR Pro | $90.21 | $74.00 | $299 | $265 (Pro bundle) |
| **(d) PRO BENCH bundle — Hub Pro + 12VHPWR Pro + 1 patch cable ($5, no feed cable: no 24-pin in this bundle, Hub runs on host USB)** | ~$112 | ~$91 | $79+299+5=**$383** sum | **$349 (-9%)** |
...
over ~$41 landed (real margin even at an aggressive 19% discount); loaded bundle retains ~$221 over
~$118 landed; Pro bench retains ~$237 over ~$112 landed. Bundling costs the platform discount
dollars, not margin ratio — every bundle here still clears >2.5× landed cost.
...
- **Consigned-part handling is a real, unresolved cost and schedule risk**, not just a price gap:
  ...
  line at all**, so JLCPCB's standard LCSC-sourced SMT flow does not cover them — either customer
  consignment (added handling fee + lead time) or a separate manual assembly step is required. This
  study's flat "assembly+consigned" line is an allowance, not a quoted consignment fee.
- **No NRE, tooling, or certification cost is amortized anywhere in this study** — stencils, fixture
  cost, and any voluntary EMC/safety testing (even for an unintentional-radiator, subassembly
  posture per §6.14) are excluded from unit landed cost.
- **No enclosure/chassis is costed** (see §0) — these are internally-mounted PCBAs, not boxed
  ...
  enclosure BOM line.
- **Channel model: direct-to-consumer assumed.** The 3.5× multiplier is sized for a DTC margin
  structure. A retail-distribution channel (distributor + retailer, each typically 30-50% margin)
  would require either a substantially higher MSRP at the same DTC margin, or a compressed DTC
  margin at the same shelf price — this study does not model that channel.

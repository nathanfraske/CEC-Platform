...
| — | **D-ENT-1..5 — Enterprise/MC production-requirements program (2026-07-01)**: (1) ratify OQ-7 scope = specify both tiers now; (2) compute architecture — §1 table "ESP32-P4 + secure element" vs Appendix B.5 PolarFire-SoC leaning (they conflict; PolarFire likely breaks the $50 target); (3) BOM re-baseline $50/$80; (4) module stance — keep tier-agnostic (recommended, LOCKED today) vs Enterprise module SKUs; (5) adopt/decline the optional module-touching items (§2.3 1-Wire ID path, CAN-FD trigger, module provenance role, mezzanine product form). Also: **re-auth the Google Drive MCP connector** (token expired) so customer/owner requirement docs can be swept into intake. | OQ-7 close; Phase 3/4 of the plan; the hub-enterprise/hub-mission-critical board programs | `docs/enterprise-mc-requirements-plan-2026-07-01.md` |

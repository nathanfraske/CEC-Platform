...
| — | **PHASE-2 SURVEYS DONE (2026-07-02) — new owner items**: (1) **CRA Annex III classification** — is the ENT-NET northbound surface a Class I "network management system"? Forks self-assessment vs notified-body; needs the Commission's delegated act or counsel, never assumption. (2) **CRA 2026-09-11 applicability** — Art. 14 PSIRT/reporting binds ANY in-scope unit EU-market-placed by that date (retroactive; includes Standard/Pro if sold in EU) — confirm market-placement status/intent. (3) **wolfSSL FIPS OE-extension** — no RISC-V on wolfCrypt's validated OE list; CMVP runs 12-18mo — start the vendor conversation at Phase 3 if FIPS posture is wanted. (4) **S-suffix confirm** — Athena coprocessor is gated to MPFS-xxx**TS** parts; confirm REQ-HUB-COMMON-001 requires it before D-ENT-3 locks the BOM line. (5) **RJ-11 ↔ OQ-60 merge** — survey 3's define-as (tamper-loop + dry-contact alarm) and OQ-60's per-port Max sideband claim the same 6P6C shell; resolve as ONE decision. (6) **NanoKVM egress on ENT-AIR** — its own Ethernet/WiFi forensic path contradicts zero-egress-by-design; gate policy-disabled-by-default or remove on AIR builds. | D-ENT-3/5/6 closure; Phase-3 register review; Phase-4 spec edit | `docs/enterprise-requirements/research/phase2/INDEX.md` |

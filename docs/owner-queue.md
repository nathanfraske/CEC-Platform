...
| — | **Placement actuator: corridor-scoped vs generalized (2026-06-16).** The actuation lever's placement arm (`apply_placement_move` → `corridor_violations` → `apply_corridor_evict`) only moves a SENSITIVE body that sits inside a FORMED high-current corridor — so `corridor_violations()` == [] for the committed Hub (shared-bus, no cables) AND the committed eps (clean placement), and `placement_moved_rate` is structurally 0 on both (EMPIRICALLY confirmed, host pcbnew; review wf_6653dbfc). The prose-ref fix (f8ddbd9) now resolves the named body correctly, and the evict chain is proven on an injected-violation EPS fixture (test_placement_actuation_e2e), but a live `placement_moved_rate > 0` needs one of: **(1)** keep the lever EPS/PCIe-corridor-scoped → validate the full finder→prose-ref→evict chain on an EPS board with an INJECTED corridor violation (bounded; my recommendation to prove the chain first); or **(2)** GENERALIZE to a non-corridor "make-room" eviction band from local congestion (GR-01 hotspot / failed-waypoint geometry) so Hub-class congestion failures move a body — larger, and the corridor-evict safety model does NOT transfer (needs its own design). | live placement actuation (`placement_moved_rate > 0`); whether the Hub can ever exercise the placement lever | FOLLOWUPS 2026-06-16 "KEY BLOCKER … CORRIDOR-GATED"; TODO.md; review wf_6653dbfc |

        ...
        for i, lens in enumerate(_LENSES):
            try:
                d = jl._chat_json(
                    ...
                    max_tokens=300, model="cec-worker", timeout=120)
...
def run(board, rounds, hours):
    ...
    while True:
        ...
        try:
            ...
            # T5 auditor -> CL-24 verifier -> guarded injection
            ...
            if rnd % V4_EVERY == 0:
                log(f"  T8 V4 batch audit ({len(batch_for_v4)} rounds; cold-start allowed)...")
